...
function generateFakeService(record) {
    const idLength = record.objectId.length;
    const recordID = record.objectId;

    let serviceID = '';

    for (let i = 0; i < idLength; i++) {
        const letterIndex = Math.floor(Math.random() * (idLength));
        
        serviceID += recordID.split('')[letterIndex];
    }

    return {
        id: serviceID,
        date: "2020-01-27T20:44:18.297Z",
        emissions: Boolean(Math.floor(Math.random() * (2))),
        combustion: Boolean(Math.floor(Math.random() * (2))),
        exterior: Boolean(Math.floor(Math.random() * (2))),
    }
}
...
for(let i = 0; i < cars.length; i++) {
    const car = cars[i];
    const maxServices = Math.floor(Math.random() * (6));
    const services = [];

    for (let j = 0; j < maxServices; j++) {
        services.push(generateFakeService(car));
    }
